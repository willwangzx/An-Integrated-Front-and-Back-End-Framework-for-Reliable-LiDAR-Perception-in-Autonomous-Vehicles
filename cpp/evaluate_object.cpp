...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  // Skip shell-based plotting commands for portability on Windows environments.
}
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // supported usage:
  //   ./eval_detection result_sha
  //   ./eval_detection result_sha num_images
  //   ./eval_detection result_sha user_sha email
  //   ./eval_detection result_sha user_sha email num_images
  if (argc!=2 && argc!=3 && argc!=4 && argc!=5) {
    cout << "Usage: ./eval_detection result_sha [num_images] [user_sha email [num_images]]" << endl;
    return 1;
  }

  // read arguments
  string result_sha = argv[1];
  int32_t arg_num_images = N_TESTIMAGES;
  if (argc==3) {
    arg_num_images = atoi(argv[2]);
  } else if (argc==5) {
    arg_num_images = atoi(argv[4]);
  }
  if (arg_num_images > 0) {
    N_TESTIMAGES = arg_num_images;
  }

  // init notification mail
  Mail *mail;
  if (argc==4 || argc==5) mail = new Mail(argv[3]);
  else         mail = new Mail();
  ...
  if (eval(result_sha,mail)) {
    ...
    if (argc==4 || argc==5)
      ...
      mail->msg("results/%s", result_sha.c_str());
  } else {
    std::filesystem::remove_all("results/" + result_sha);
