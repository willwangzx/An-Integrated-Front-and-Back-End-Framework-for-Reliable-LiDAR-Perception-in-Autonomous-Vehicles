...
> 注意：当前入口脚本文件名为 `main,py`（文件名中是逗号而不是点）。如果这是笔误，建议后续重命名为 `main.py` 以便维护。
...
如果你需要执行 `data_processing/kitti_bin_to_las.py`，还需要安装：

```bash
pip install pdal
```

> `pdal` 在不同平台的安装差异较大，建议优先参考 PDAL 官方文档或使用 Conda 环境安装。
